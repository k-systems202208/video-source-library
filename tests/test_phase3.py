...
class Phase3ScannerTests(unittest.TestCase):
    ...
    def test_scan_discoveries_survives_later_schema_migrations(self):
        with connect(self.db_path) as c:
            initialize_database(c); self.assertEqual(c.execute('SELECT schema_version FROM schema_info').fetchone()[0],4); self.assertEqual(SCHEMA_VERSION,4); self.assertIsNotNone(c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='scan_discoveries'").fetchone())
